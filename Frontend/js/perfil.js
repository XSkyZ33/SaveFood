...
function calcularPreferencias(marcacoes, pratosMap) {
    ...
    marcacoes.forEach(m => {
        if (m.estado !== 'servido') return;

        const prato = pratosMap[m.prato];
        if (!prato) return;

        totalServidos++;
        const tipo = prato.tipo || 'Outro';
    ...
    Object.entries(tiposCount).forEach(([tipo, count]) => {
        porcentagens[tipo] = Math.round((count / totalServidos) * 100);
    });
...
function renderPreferencias(prefs) {
    ...
    for (const tipo in prefs) {
        const perc = prefs[tipo];
        const cor = cores[tipo] || cores['Outro'];
  ...
  marcacoes.forEach(m => {
    ...
    row.innerHTML = `
      ...
      <td class="px-6 py-4 whitespace-nowrap">
        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">${prato.tipo || 'Outro'}</span>
      </td>
      <td class="px-6 py-4 whitespace-nowrap">
